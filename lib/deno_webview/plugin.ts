...
import { deferred } from "../deno_std/async/deferred.ts";
import { parse as parsePath } from "../deno_std/path/mod.ts";
...
export async function load() {
  unload();
  pluginId = Deno.openPlugin(
    decodeURI(parsePath(import.meta.url).dir.replace("file:///",""))+"/deno_webview.dll"
  );
}
